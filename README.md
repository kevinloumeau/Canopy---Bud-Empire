...
A mobile-first dispensary idle game built with plain JavaScript, Vite and an isometric Canvas 2D renderer. Grow and pack products, serve walk-in customers, expand the ordering team, dispatch deliveries and build a network of neighborhood stores.

The shop uses a fixed camera orientation with pan and zoom, animated staff and customers, and day/night lighting. Game progress stays in the browser; no account or game server is required.

## Run locally

Use Node.js 22.12 or newer. The pinned Vite version also supports Node 20.19+.
...
Open [the development preview](http://127.0.0.1:4180/). Without the port override, the package scripts default to port 4173.

To build and inspect the production version in a separate terminal:
...
Open [the production preview](http://127.0.0.1:4181/). Building or running a preview does not publish the game.

The build targets Safari 14 and runs `scripts/make-classic.mjs` to replace Vite's module entry with a deferred classic script. Preserve this build step as part of the established mobile compatibility path. Asset URLs are relative so the output also works under a subdirectory.
