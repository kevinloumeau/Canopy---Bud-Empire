...
export function prestigeOffer(state){const rank=state.empire.prestige||0,target=1000000*3**Math.min(rank,18);return {rank,target,eligible:rank<19&&state.money>=target,multiplier:1+(rank+1)*.2};}
...
export function makeSound(){let context,last=0;return function(kind,enabled){if(!enabled||document.hidden)return;try{const now=Date.now();if(now-last<120)return;last=now;const Audio=window.AudioContext||window.webkitAudioContext;if(!Audio)return;context=context||new Audio();if(context.state==='suspended')context.resume().catch(()=>{});const osc=context.createOscillator(),gain=context.createGain(),t=context.currentTime;osc.type='sine';osc.frequency.setValueAtTime(kind==='sale'?880:523,t);osc.frequency.exponentialRampToValueAtTime(kind==='sale'?1320:1046,t+.1);gain.gain.setValueAtTime(.025,t);gain.gain.exponentialRampToValueAtTime(.001,t+.18);osc.connect(gain);gain.connect(context.destination);osc.start(t);osc.stop(t+.2)}catch(e){/* Audio is optional when the browser denies playback. */}}}
