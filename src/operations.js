...
export const BRANCH_NAMES=['Riverside','Old Town','City Center','Desert Oasis','Alpine'];
export const LINES=[
...
export const REGULARS=[
 {name:'Mara',role:'Riverwalk gardener',requests:['A jar for the gardening club','Tins for the neighborhood swap','A reserve box for the club anniversary'],
  lines:{meet:'You must be the new grower. The club has been talking about this place all week.',request:'Could you set something aside for us? The club meets tonight by the water.',thanks:'They loved it. I pressed a cutting for you — it is on the counter.',regular:'Morning. The usual bench, the usual order. It is good to have a place like this.'}},
 {name:'Sol',role:'Local printmaker',requests:['Something simple for the studio','Tins for the gallery opening','A reserve box for a first collector'],
  lines:{meet:'I have a studio two doors down. The light in here is better than mine, honestly.',request:'Opening night is Friday. Something for the table that says we are serious?',thanks:'The prints sold out. I owe this shop a poster — no, a mural.',regular:'Back again. I brought you a proof of the new print. Ink is still drying.'}},
 {name:'Kit',role:'Bike courier',requests:['A quick pickup after the route','Tins for the courier crew','A reserve box for a hundredth delivery'],
  lines:{meet:'Fastest route in the city ends at your door. That is not an accident.',request:'The crew finishes at six. Can you have something boxed for the lot of us?',thanks:'Crew says thanks. You are on the good list — deliveries here jump the queue.',regular:'One hundred and counting. Same order, and do not tell me to slow down.'}},
 {name:'Ruth',role:'Desert botanist',requests:['A jar for the field station','Tins for the stargazing circle','A reserve box for the bloom festival'],
  lines:{meet:'Your terrace agaves are healthier than the ones at my field station. Impressive.',request:'The stargazing circle meets after dark. We will want something for the cold.',thanks:'The bloom festival committee sends its regards. And its seed catalogue.',regular:'Evening. Two things: your saguaro needs nothing, and I need the usual.'}},
 {name:'Nils',role:'Trail guide',requests:['A jar for the summit cabin','Tins for the ski patrol','A reserve box for the solstice bonfire'],
  lines:{meet:'Saw your lights from the ridge. Good sign, a warm window in the pines.',request:'Patrol rotates Thursday. Something warming for the cabin shelf?',thanks:'The bonfire went past midnight. You would have liked it. Next year, you come.',regular:'Trail is clear, lodge is warm, and I will take the usual before the light goes.'}}];
...
export function migrateOperations(raw){
 ...
 const stores=BRANCH_NAMES.map((_,i)=>{const s=r.stores?.[i]||{},shelves=triple(s.shelves,60);if(!shelves.some(Boolean))shelves[0]=20;while(shelves.reduce((a,b)=>a+b,0)>60){const j=shelves.findIndex(n=>n>0);shelves[j]--;}
...
export function transferReason(p,to,amount=10){const n=p.network;if(![1,2,3,4].includes(to)||!p.stores[0].level||!p.stores[to].level)return 'Open both stores';if(n.jobs.length>=n.fleet)return 'All drivers busy';if(n.stores[0].raw<amount)return 'Need '+amount+' harvest';const reserved=n.jobs.filter(j=>j.kind==='transfer'&&j.to===to).reduce((a,j)=>a+j.amount,0);return n.stores[to].raw+reserved+amount>120?'Destination full':'';}
...
export function flagshipStatus(state){const p=state.empire,n=p.network,entries=journal(state);const checks=[{name:'Store quality',value:p.stores.filter(s=>s.level>=7&&s.projects.filter(Boolean).length>=2).length,total:p.stores.length},{name:'Collection',value:entries.filter(e=>e.found).length,total:entries.length},{name:'Grand openings',value:n.stores.filter(s=>s.opening===3).length,total:n.stores.length},{name:'Reputation',value:Math.min(60,p.reputation,...p.stores.map(s=>s.loyalty)),total:60}];return {checks,ready:checks.every(c=>c.value>=c.total),claimed:n.flagship};}
