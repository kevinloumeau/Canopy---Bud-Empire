(function () {
  ...
  function renderUI(){var line=LINES[selected],open=state.lifetime>=line.unlock,level=state.lines[selected];$('money').textContent=fmt(state.money);$('rate').textContent=fmt(production())+'/s';$('tapValue').textContent='+'+fmt(tapValue());$('machineNumber').textContent=open?'MACHINE 0'+(selected+1):'UNLOCK AT '+fmt(line.unlock);$('machineName').textContent=line.name;$('machineLevel').textContent=level;$('machineRate').textContent=fmt(line.rate*level*state.multiplier)+'/s';$('machineCost').textContent=fmt(cost(selected));$('buyMachine').disabled=!open;$('buyMachine').querySelector('span').textContent=level?'UPGRADE':'BUILD';var order=ORDERS[state.contract];if(order){$('orderName').textContent=order.name;$('orderProgress').textContent=fmt(Math.min(state.lifetime,order.goal))+' / '+fmt(order.goal);$('orderFill').style.width=Math.min(100,state.lifetime/order.goal*100)+'%';$('claimReward').textContent='+'+fmt(order.reward);$('claim').disabled=state.lifetime<order.goal}else{$('orderName').textContent='ALL ORDERS FILLED';$('orderProgress').textContent='COMPLETE';$('orderFill').style.width='100%';$('claimReward').textContent='✓';$('claim').disabled=true}$('boostCost').textContent=fmt(Math.floor(250*Math.pow(4,state.globalLevel)));markerEls.forEach(function(el,i){el.classList.toggle('selected',i===selected);el.classList.toggle('locked',state.lifetime<LINES[i].unlock);el.textContent=state.lifetime<LINES[i].unlock?fmt(LINES[i].unlock):LINES[i].name})}
  ...
  resize();renderUI();ctx.fillStyle=colors.bg;ctx.fillRect(0,0,width,height);window.__shiftReady=true;playLoadSequence();requestAnimationFrame(render);setTimeout(function(){$('gestureHint').style.opacity=.25},5000);setInterval(function(){add(production()/4);renderUI()},250);setInterval(save,5000);window.addEventListener('beforeunload',save);
